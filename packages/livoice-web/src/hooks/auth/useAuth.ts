...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const canEditLocation = useMemo(() => canEditLocationData(user), [user]);
  ...
  const contextValue = useMemo(
    () => ({
      ...
      canEditLocationData: (userOrSession?: ExtendedSession | null) => canEditLocationData(userOrSession ?? user),
    ...
    [user, isLoadingAuth, canAdmin, canEditOrg, canEditLocation]
  );
