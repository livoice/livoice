...
const TranscriptPage = () => {
  ...
  const transcriptQuery = useGetTranscriptQuery(transcriptId, { enabled: !!transcriptId });
  const chunkQuery = useListTranscriptChunksQuery(transcriptId, { enabled: !!transcriptId });
  ...
  const handleUpload = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await uploadMutation.mutateAsync({ id: transcriptId, body: { text: transcriptText } });
    setTranscriptText('');
  };

  const handleChat = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!transcriptId || !event.currentTarget.elements.namedItem('chatMessage')) return;
    const formData = new FormData(event.currentTarget);
    const message = (formData.get('chatMessage') as string | null) ?? '';
    if (!message.trim()) return;

    const response = await chatMutation.mutateAsync({ id: transcriptId, body: { userMessage: message } });
  ...
  if (transcriptQuery.isError || !normalizedTranscript) return <Card>{t('status.error')}</Card>;
        ...
        <div className="space-y-6 px-8 py-8">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <h1 className="text-3xl font-semibold">{normalizedTranscript.title}</h1>
              <p className="max-w-3xl text-white/80">{normalizedTranscript.description}</p>
            </div>
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                { label: t('fields.date'), value: formattedDate || '—' },
                { label: t('fields.interviewee'), value: normalizedTranscript.intervieweeName ?? '—' },
                { label: t('fields.chunks'), value: normalizedTranscript.chunkCount ?? 0 }
              ].map(stat => (
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr,1fr]">
        <Card className="space-y-6">
          <form onSubmit={handleUpload} className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.4em] text-slate-400">Upload transcript</p>
                <p className="text-sm text-slate-500">Paste the interview text to keep the knowledge base fresh.</p>
              </div>
        ...
        <div className="space-y-6">
            ...
            <form onSubmit={handleChat} className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">Ask any follow-up question about this transcript.</p>
                </div>
          ...
          <Card className="space-y-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.4em] text-slate-400">Chat history</p>
              <p className="text-sm text-slate-500">Review the ongoing conversation with this transcript.</p>
            </div>
            {chatHistory.length ? (
